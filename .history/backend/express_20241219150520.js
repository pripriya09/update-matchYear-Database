...
app.post('/update-year', async (req, res) => {
    ...
    try {
      ...
      const result = await User.updateMany(
        ...
        { $set: { effective_date: { $replaceOne: { input: "$effective_date", find: oldYear, replacement: newYear } } } } // Replace year
      );
  
      if (result.modifiedCount > 0) {
        return res.status(200).json({ message: 'Effective dates updated successfully.' });
      } else {
